...
public:
    ...
    int getPred(int idx);
    AdminData getPredAdmin(int idx);

    // A function to get the successor of the key- where the key
    // is present in the idx-th position in the node
    int getSucc(int idx);
    AdminData getSuccAdmin(int idx);
...
void BTreeNodeAdmin::traverse()
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (!leaf)
            C[i]->traverse();
        cout << " " << keys[i] <<"["<<adminDataArray[i].getName()<<"]";
    }
...
int BTreeNodeAdmin::findKey(int k)
{
    int idx=0;
...
void BTreeNodeAdmin::remove(int k)
{
    int idx = findKey(k);
...
void BTreeNodeAdmin::removeFromLeaf (int idx)
{
...
void BTreeNodeAdmin::removeFromNonLeaf(int idx)
{
    ...
    if (C[idx]->n >= t)
    {
        ...
        AdminData predAdmin = getPredAdmin(idx);
        keys[idx] = pred;
        adminDataArray[idx] = predAdmin;
        C[idx]->remove(pred);
    }
    ...
    else if  (C[idx+1]->n >= t)
    {
        ...
        AdminData succAdmin = getSuccAdmin(idx);
        keys[idx] = succ;
        adminDataArray[idx] = succAdmin;
        C[idx+1]->remove(succ);
    }
...
int BTreeNodeAdmin::getPred(int idx)
{
    // Keep moving to the right most node until we reach a leaf
    BTreeNodeAdmin *cur=C[idx];
    while (!cur->leaf)
        cur = cur->C[cur->n];
...
AdminData BTreeNodeAdmin::getPredAdmin(int idx)
{
    // Keep moving to the right most node until we reach a leaf
    BTreeNodeAdmin *cur=C[idx];
    while (!cur->leaf)
    ...
    // Return the last key of the leaf
    return cur->adminDataArray[cur->n-1];
}

AdminData BTreeNodeAdmin::getSuccAdmin(int idx){

    // Keep moving the left most node starting from C[idx+1] until we reach a leaf
    BTreeNodeAdmin *cur = C[idx+1];
    while (!cur->leaf)
        cur = cur->C[0];

    // Return the first key of the leaf
    return cur->adminDataArray[0];
}

int BTreeNodeAdmin::getSucc(int idx)
{
...
void BTreeNodeAdmin::fill(int idx)
{
...
// into C[idx]
void BTreeNodeAdmin::borrowFromPrev(int idx)
{

    BTreeNodeAdmin *child=C[idx];
    BTreeNodeAdmin *sibling=C[idx-1];
    ...
    for (int i=child->n-1; i>=0; --i) {
        ...
        child->adminDataArray[i+1] = child->adminDataArray[i];
    }
    ...
    child->adminDataArray[0] = adminDataArray[idx-1];
    ...
    adminDataArray[idx-1] = sibling->adminDataArray[sibling->n-1];
...
// it in C[idx]
void BTreeNodeAdmin::borrowFromNext(int idx)
{
    ...
    child->adminDataArray[(child->n)] = adminDataArray[idx];
    ...
    adminDataArray[idx] = sibling->adminDataArray[0];

    // Moving all keys in sibling one step behind
    for (int i=1; i<sibling->n; ++i) {
        sibling->keys[i - 1] = sibling->keys[i];
        sibling->adminDataArray[i - 1] = sibling->adminDataArray[i];
    }
...
void BTreeNodeAdmin::merge(int idx)
{
    BTreeNodeAdmin *child = C[idx];
    BTreeNodeAdmin *sibling = C[idx+1];
    ...
    child->adminDataArray[t-1] = adminDataArray[idx];

    // Copying the keys from C[idx+1] to C[idx] at the end
    for (int i=0; i<sibling->n; ++i) {
        child->keys[i + t] = sibling->keys[i];
        child->adminDataArray[i + t] = sibling->adminDataArray[i];
    }
