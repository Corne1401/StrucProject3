...
class ArticulationPointGraph {
    ...
    void APT(int v, bool visited[], int dis[], int low[],
             int par[], bool ap[]);
public:
    ArticulationPointGraph(int n); //constructor
    void addEd(int w, int x);
    void AP();
};
ArticulationPointGraph::ArticulationPointGraph(int n) {
    this->n = n;
    adj = new list<int>[n];
}
//add edges to the graph
void ArticulationPointGraph::addEd(int w, int x) {
    adj[x].push_back(w); //add u to v's list
    adj[w].push_back(x); //add v to u's list
}
void ArticulationPointGraph::APT(int w, bool visited[], int dis[], int low[], int par[], bool ap[]) {
    static int t=0;
...
void ArticulationPointGraph::AP() {
